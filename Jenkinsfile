...
"""
    ...
    node (label) {
        stage ('Checkout SCM'){
          git credentialsId: 'git', url: 'https://github.com/rajtaldevop1/eos-cloud-config-api.git', branch: 'main'
          container('build') {
          ...
          container('build') {
                stage('Deploy Artifacts') {
                  // sh 'chmod 777 /home/jenkins/agent/workspace/eos-registry-api_main/mvnw'
                    sh ' chmod 777 /home/jenkins/agent/workspace/eos-cloud-config-api/mvnw'
                    rtMavenRun (
       ...
       stage ('Docker Build'){
          container('build') {
                stage('Build Image') {
                    ...
                    def customImage = docker.build("rajeshtalla0209/eos-cloud-config-api:latest")
                    customImage.push()             
                    }
                }
            }
        ...
        stage ('Helm Chart') {
          container('build') {
            dir('charts') {
              ...
              sh '/usr/local/bin/helm push-artifactory cloud-config-api-1.0.tgz https://rajdev.jfrog.io/artifactory/eos-helm-local --username $username --password $password'
              }
            }
        }
        }
